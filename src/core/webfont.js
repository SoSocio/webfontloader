...
goog.scope(function () {
	...
	WebFont.prototype.getFontApiUrl = function (configuration) {
		const context = configuration['context'] || this.mainWindow_;
		this.domHelper_ = new DomHelper(this.mainWindow_, context);
		const modules = this.fontModuleLoader_.getModules(configuration, this.domHelper_);
		const urls = [];

		for (var i = 0, len = modules.length; i < len; i++) {
			const module = modules[i];
			const url = module.getFontApiUrl();
